...
@elseif (strstr($response['content'], '"headers"'))
{!! json_decode($response['content'], true)['headers'] ?? '' !!}
@else
...
@elseif (is_string($route['response']) && !strstr($route['response'], '{'))
{{ $route['response'] }}
@else
...
@foreach($route['bodyParameters'] as $attribute => $parameter)
    {{$attribute}} | {{$parameter['type']}} | @if($parameter['required']) (required) @endif | {!! $parameter['description'] !!}
@endforeach
@endif
...
@foreach($route['queryParameters'] as $attribute => $parameter)
    {{$attribute}} | @if($parameter['required']) (required) @endif | {!! $parameter['description'] !!}
@endforeach
@endif
